...
const ManageCaregivers = () => {
  ...
  const fetchCaregivers = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/admin/caregivers');
  ...
  const fetchKYCDetails = async (caregiverId) => {
    setLoadingKyc(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/admin/kyc/${caregiverId}`);
  ...
  const handleKycAction = async (action) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:5000/api/admin/kyc/${selectedCaregiver.id}/status`,
        {
  ...
  const handleUpdateStatus = async () => {
    try {
      await axios.post('http://localhost:5000/api/admin/update-caregiver-status', {
  ...
  const handleDeleteCaregiver = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/admin/delete-caregiver/${selectedCaregiver.id}`);
              ...
              <Box mb={3}>
                ...
                <Grid container spacing={2} sx={{ mt: 1 }}>
                  <Grid item xs={12} md={6}>
                    ...
                    <img 
                      src={`http://localhost:5000/uploads/kyc/${kycDetails.id_card_path}`}
                      alt="ID Card"
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <img 
                      src={`http://localhost:5000/uploads/kyc/${kycDetails.proof_of_address_path}`}
              ...
              <Box mb={3}>
                ...
                <Grid container spacing={2} sx={{ mt: 1 }}>
                  ...
                  <Grid item xs={12}>
                    ...
                    <img 
                      src={`http://localhost:5000/uploads/kyc/${kycDetails.guarantor1_document_path}`}
              ...
              <Box mb={3}>
                ...
                <Grid container spacing={2} sx={{ mt: 1 }}>
                  ...
                  <Grid item xs={12}>
                    <Typography variant="subtitle2">Guarantor Document</Typography>
                    <img 
                      src={`http://localhost:5000/uploads/kyc/${kycDetails.guarantor2_document_path}`}
