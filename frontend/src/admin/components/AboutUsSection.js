...
const AboutUsSection = () => {
  ...
  useEffect(() => {
    const fetchAbout = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/admin/get-about');
        if (res.data.length > 0) {
          ...
          setAboutData({
            ...
            imageUrl: `http://localhost:5000/uploads/about/${about.about_image}`,
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/admin/add-about', formData);
