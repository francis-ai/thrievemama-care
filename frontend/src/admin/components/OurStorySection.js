...
const OurStorySection = () => {
  ...
  useEffect(() => {
    const fetchStory = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/admin/get-story');
        if (res.data.length > 0) {
          ...
          setData({
            ...
            imageUrl: `http://localhost:5000/uploads/story/${item.story_image}`,
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/admin/add-story', formData);
