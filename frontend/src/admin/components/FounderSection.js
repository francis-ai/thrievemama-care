...
const FounderSection = () => {
  ...
  useEffect(() => {
    const fetchFounder = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/admin/get-founder');
        if (res.data.length > 0) {
          ...
          setFounderData({
            ...
            imageUrl: `http://localhost:5000/uploads/founder/${founder.founder_image}`,
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/admin/add-founder', formData);
