    ...
    async function fetchSections() {
      try {
        const [storyRes, founderRes] = await Promise.all([
          axios.get('http://localhost:5000/api/admin/get-story'),
          axios.get('http://localhost:5000/api/admin/get-founder'),
        ]);
        if (storyRes.data.length) {
          ...
          setStory({
            title: s.title,
            content: s.content,
            imageUrl: `http://localhost:5000/uploads/story/${s.story_image}`,
          });
        }
        if (founderRes.data.length) {
          ...
          setFounder({
            title: f.title,
            content: f.content,
            imageUrl: `http://localhost:5000/uploads/founder/${f.founder_image}`,
          });
        }
      } catch (err) {
    ...
    <div className="About-container">
      ...
      <Container className="about-us my-5">
        <Row className="text-center mb-5" data-aos="fade-down">
          <Col><h1 className="section-title">Empowering Families, One Caregiver at a Time</h1></Col>
        </Row>
        ...
        <Row className="mt-5 text-center" data-aos="fade-up">
          <Col>
            ...
            <p>
              Whether you're a parent, a family caregiver, or a professional looking to support others, you're welcome here.<br />
              We're growing a supportive community where care is not just a service, but a way of life.
            </p>
            <p>
