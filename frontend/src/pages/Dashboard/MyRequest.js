...
const MyRequests = () => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/users/my-caregiver-requests/${userId}`);
  ...
  const submitEditRequest = async () => {
    try {
      await axios.put(
        `http://localhost:5000/api/users/update-my-caregiver-requests/${editRequestData.id}`,
        editRequestData
      );
  ...
  const confirmDeleteRequest = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/users/delete-my-caregiver-requests/${requestToDelete}`);
