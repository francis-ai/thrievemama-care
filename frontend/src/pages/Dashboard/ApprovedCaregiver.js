...
const ApprovedCaregiver = () => {
  ...
  const fetchApprovedCaregivers = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/users/approved-caregivers/${userId}`);
      const caregiversWithReviews = await Promise.all(
        res.data.map(async (item) => {
          try {
            const reviewRes = await axios.get(
              `http://localhost:5000/api/users/caregiver-review/${userId}/${item.caregiver_id}`
            );
            return { 
              ...item, 
  ...
  const fetchAppointments = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/users/appointments/${userId}`);
      ...
      const paymentHandler = window.PaystackPop.setup({
        ...
        callback: function(response) {
          (async () => {
            try {
              // Verify payment
              await axios.post('http://localhost:5000/api/users/verify-payment', {
              ...
              // Create appointment
              await axios.post('http://localhost:5000/api/users/create-appointment', {
  ...
  const handleCompleteAppointment = async (appointmentId) => {
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/users/complete-appointment/${appointmentId}`,
        { status: 'Completed' }
      );
  ...
  const handleSubmitReview = async () => {
    try {
      await axios.post('http://localhost:5000/api/users/caregiver-review', {
        user_id: userId,
  ...
  const handleSaveEdit = async (caregiverId) => {
    try {
      await axios.put(
        `http://localhost:5000/api/users/caregiver-review/${userId}/${caregiverId}`,
  ...
  return (
    <DashboardLayout>
      <Box p={3}>
        <Typography variant="h5" mb={3}>Approved Caregivers</Typography>

        {caregivers.length === 0 ? (
          <Typography>No approved caregivers found.</Typography>
        ) : (
          caregivers.map((item) => {
                    ...
                    <Box display="flex" alignItems="center" gap={2}>
                      <img
                        src={item.caregiver_image
                          ? `http://localhost:5000/uploads/caregivers/${item.caregiver_image}`
                          : '/avatar.png'}
