...
const Support = () => {
    ...
    const fetchTickets = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/auth/get-support-tickets?user_id=${caregiverId}&user_type=caregiver`
        );
  ...
  const handleSubmitTicket = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/auth/submit-ticket', {
        method: 'POST',
  ...
  const fetchSingleTicket = async (ticketId) => {
    try {
      const res = await fetch(`http://localhost:5000/api/auth/support-tickets/${ticketId}?user_id=${caregiverId}&user_type=caregiver`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleSendMessage = async (msg) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/auth/support-tickets/${selectedTicket.id}/reply?user_id=${caregiverId}&user_type=caregiver`,
        {
          method: 'POST',
          headers: {
