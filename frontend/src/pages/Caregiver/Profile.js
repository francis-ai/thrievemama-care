...
const MyAccount = () => {
  ...
  useEffect(() => {
    ...
    const fetchcaregiverProfile = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/caregivers/profile/${caregiver.id}`);
        ...
        if (caregiverData.profile_image) {
          setPreview(`http://localhost:5000/uploads/caregivers/${caregiverData.profile_image}`);
        }
      } catch (err) {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/caregivers/update-profile', form);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/caregivers/change-password', {
        id: caregiverId,
