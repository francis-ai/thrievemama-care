...
const KYC = () => {
  ...
  useEffect(() => {
    const checkKYCStatus = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/caregivers/kyc/${caregiverId}`);
        if (res.data.success) {
          ...
          if (res.data.data.status === 'Rejected') {
            ...
            setFilePreviews({
              idCard: kycData.id_card_path ? `http://localhost:5000/uploads/kyc/${kycData.id_card_path}` : null,
              proofOfAddress: kycData.proof_of_address_path ? `http://localhost:5000/uploads/kyc/${kycData.proof_of_address_path}` : null,
              guarantor1_document: kycData.guarantor1_document_path ? `http://localhost:5000/kyc/uploads/${kycData.guarantor1_document_path}` : null,
              guarantor2_document: kycData.guarantor2_document_path ? `http://localhost:5000/kyc/uploads/${kycData.guarantor2_document_path}` : null
            });
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = status === 'Rejected' 
        ? `http://localhost:5000/api/caregivers/kyc/caregiver/${caregiverId}`
        : 'http://localhost:5000/api/caregivers/kyc';
