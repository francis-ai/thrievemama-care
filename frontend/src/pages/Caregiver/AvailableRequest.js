...
const AvailableRequest = () => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      const [requestsRes, interestsRes] = await Promise.all([
        axios.get('http://localhost:5000/api/caregivers/all-caregiver-requests'),
        axios.get(`http://localhost:5000/api/caregivers/my-interests/${caregiverId}`)
      ]);
  ...
  const toggleInterest = async (requestId) => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/caregivers/${alreadyInterested ? 'unshow' : 'show'}-interest`,
